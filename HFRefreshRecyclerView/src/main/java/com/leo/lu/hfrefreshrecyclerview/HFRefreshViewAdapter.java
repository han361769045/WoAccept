...
import com.leo.lu.hfrefreshrecyclerview.itemTouchHelper.ItemTouchHelperAdapter;
import com.leo.lu.hfrefreshrecyclerview.stickyheadersrecyclerview.StickyRecyclerHeadersAdapter;
 ...
 */
public abstract class HFRefreshViewAdapter<VH extends RecyclerView.ViewHolder> extends RecyclerView.Adapter<VH> implements StickyRecyclerHeadersAdapter<RecyclerView.ViewHolder>, ItemTouchHelperAdapter {
    protected Handler timer = new Handler();
    protected HFRefreshRecyclerView.CustomRelativeWrapper customHeaderView = null;
     ...
     */
    public void setCustomHeaderView(HFRefreshRecyclerView.CustomRelativeWrapper customHeaderView) {
        this.customHeaderView = customHeaderView;
        customHeader = true;
    }

    public HFRefreshRecyclerView.CustomRelativeWrapper getCustomHeaderView() {
    ...
    protected boolean detectDispatchLoadMoreDisplay(final int data_size_before_remove, final int display_size_before_remove) {
        if (data_size_before_remove == 0) {
            if (display_size_before_remove == 2) {
                ...
                if (mEmptyViewPolicy == HFRefreshRecyclerView.EMPTY_KEEP_HEADER_AND_LOARMORE) {

                } else if (mEmptyViewPolicy == HFRefreshRecyclerView.EMPTY_KEEP_HEADER) {
                    removeDispatchLoadMoreView();
                } else if (mEmptyViewPolicy == HFRefreshRecyclerView.EMPTY_CLEAR_ALL) {
                    removeDispatchLoadMoreView();
                }
                ...
                if (mEmptyViewPolicy == HFRefreshRecyclerView.EMPTY_KEEP_HEADER_AND_LOARMORE) {
                    ...
                    notifyDataSetChanged();
                } else if (mEmptyViewPolicy == HFRefreshRecyclerView.EMPTY_SHOW_LOADMORE_ONLY) {
                    notifyDataSetChanged();
                }
                return true;
            } else {
